...
class TestReturnToPoolUntilWindow:
    def test_sets_proximo_intento(self):
        conn = AsyncMock()
        with patch("app.jobs.outbox_batch_fetcher.seconds_until_next_window", return_value=3600):
    ...
    def test_uses_calculated_seconds(self):
        conn = AsyncMock()
        with patch("app.jobs.outbox_batch_fetcher.seconds_until_next_window", return_value=7200):
            run(return_to_pool_until_window(conn, _make_raw_row(), "08:00"))
