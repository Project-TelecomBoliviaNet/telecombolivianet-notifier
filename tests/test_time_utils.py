...
from app.jobs.outbox_worker import is_within_window, seconds_until_next_window, now_bolivia
...
class TestIsWithinWindow:
    def _patch(self, hour, minute):
        return patch("app.jobs.outbox_worker.now_bolivia",
...
class TestSecondsUntilNextWindow:
    def test_window_later_today(self):
        ...
        with patch("app.jobs.outbox_worker.now_bolivia", return_value=fixed):
