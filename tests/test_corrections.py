...
# ── FIX-12: INSERT atómico ON CONFLICT en outbox de recordatorios ────────────

class TestFix12_AtomicOutboxInsert:
    def test_insert_outbox_safe_query_exists(self):
        assert hasattr(Q, "INSERT_OUTBOX_SAFE"), \
            "queries.py debe tener INSERT_OUTBOX_SAFE"

    def test_insert_outbox_safe_has_on_conflict(self):
        assert "ON CONFLICT" in Q.INSERT_OUTBOX_SAFE
        assert "DO NOTHING" in Q.INSERT_OUTBOX_SAFE

    def test_insert_outbox_safe_has_returning(self):
        assert "RETURNING" in Q.INSERT_OUTBOX_SAFE

    def test_process_invoice_reminder_does_not_use_exists_reminder_log(self):
        """FIX-12: no debe haber una comprobación previa en NotifLogs."""
        import inspect
        from app.jobs.reminder_persistence import process_invoice_reminder
        src = inspect.getsource(process_invoice_reminder)
        assert "EXISTS_REMINDER_LOG" not in src, \
            "process_invoice_reminder no debe usar EXISTS_REMINDER_LOG (usar INSERT_OUTBOX_SAFE)"

    def test_process_invoice_reminder_uses_insert_outbox_safe(self):
        import inspect
        from app.jobs.reminder_persistence import process_invoice_reminder
        src = inspect.getsource(process_invoice_reminder)
        assert "INSERT_OUTBOX_SAFE" in src


# ── FIX-14: SIGTERM libera batch en limbo ────────────────────────────────────

class TestFix14_SigtermReleaseBatch:
    def test_release_batch_query_exists(self):
        assert hasattr(Q, "RELEASE_BATCH"), "queries.py debe tener RELEASE_BATCH"

    def test_release_batch_reverts_publicado_to_false(self):
        assert "Publicado" in Q.RELEASE_BATCH
        assert "FALSE" in Q.RELEASE_BATCH
        assert "ANY" in Q.RELEASE_BATCH

    def test_release_batch_only_affects_non_finalized(self):
        assert "EstadoFinal" in Q.RELEASE_BATCH
        assert "IS NULL" in Q.RELEASE_BATCH

    def test_process_batch_catches_cancelled_error(self):
        import inspect
        from app.jobs.outbox_worker import _process_batch
        src = inspect.getsource(_process_batch)
        assert "CancelledError" in src, \
            "_process_batch debe capturar CancelledError y liberar el batch"
        assert "RELEASE_BATCH" in src

    def test_process_batch_reraises_cancelled_error(self):
        """Tras liberar el batch, CancelledError debe re-lanzarse para propagar el shutdown."""
        import inspect
        from app.jobs.outbox_worker import _process_batch
        src = inspect.getsource(_process_batch)
        # 'raise' debe aparecer tras el RELEASE_BATCH
        release_idx = src.index("RELEASE_BATCH")
        raise_idx   = src.index("raise", release_idx)
        assert raise_idx > release_idx

    def test_docker_compose_has_stop_grace_period(self):
        with open("docker-compose.yml") as f:
            content = f.read()
        assert "stop_grace_period" in content, \
            "docker-compose.yml debe definir stop_grace_period para dar tiempo al rollback"


# ── FIX-15: una conexión por tipo en reminder loop ────────────────────────────

class TestFix15_SingleConnectionPerReminderType:
    def test_process_reminder_type_uses_single_connection(self):
        import inspect
        from app.jobs.reminder_persistence import process_reminder_type
        src = inspect.getsource(process_reminder_type)
        # Debe haber exactamente UN 'async with pool.acquire()' (no N+1)
        count = src.count("async with pool.acquire()")
        assert count == 1, \
            f"process_reminder_type debe tener exactamente 1 adquisición de pool, encontradas: {count}"

    def test_fetch_and_loop_within_same_connection(self):
        """El FETCH_INVOICES y el for loop deben estar dentro del mismo bloque with."""
        import inspect
        from app.jobs.reminder_persistence import process_reminder_type
        src = inspect.getsource(process_reminder_type)
        acquire_idx     = src.index("async with pool.acquire()")
        fetch_idx       = src.index("FETCH_INVOICES_DUE_IN_DAYS", acquire_idx)
        loop_idx        = src.index("for inv in invoices", fetch_idx)
        # Si el for loop viene después del fetch dentro del mismo bloque, están alineados
        assert loop_idx > fetch_idx, \
            "El for loop debe estar dentro del mismo bloque 'async with pool.acquire()'"


# ── FIX-13: statement_timeout en advisory lock ────────────────────────────────

class TestFix13_AdvisoryLockTimeout:
    def test_job_max_duration_constant_exists(self):
        from app.jobs.reminder_job import _JOB_MAX_DURATION_MS
        assert _JOB_MAX_DURATION_MS > 0

    def test_try_run_job_sets_statement_timeout(self):
        """_try_run_job debe llamar a SET statement_timeout antes de adquirir el lock."""
        import inspect
        from app.jobs.reminder_job import _try_run_job
        src = inspect.getsource(_try_run_job)
        assert "statement_timeout" in src, \
            "_try_run_job debe fijar statement_timeout en la conexión del lock"

    def test_statement_timeout_set_before_lock(self):
        """statement_timeout debe aparecer antes de try_acquire_lock en el código."""
        import inspect
        from app.jobs.reminder_job import _try_run_job
        src = inspect.getsource(_try_run_job)
        timeout_idx = src.index("statement_timeout")
        lock_idx    = src.index("try_acquire_lock")
        assert timeout_idx < lock_idx, \
            "statement_timeout debe configurarse antes de intentar adquirir el lock"
...
# ── FIX-23: Circuit Breaker para Meta API ────────────────────────────────────

class TestFix23_CircuitBreaker:
    ...
    def test_circuit_opens_after_fail_max_errors(self):
        from app.services.circuit_breaker import CircuitBreaker, CircuitBreakerError

        async def failing():
            raise RuntimeError("meta down")

        breaker = CircuitBreaker(fail_max=3, reset_timeout=60, name="test")
        ...
        async def _run():
            for _ in range(3):
                try:
                    ...
                    pass
            # Circuito debe estar abierto ahora
    ...
    def test_circuit_resets_after_success(self):
        ...
        breaker = CircuitBreaker(fail_max=3, reset_timeout=0, name="test")

        async def _run():
            await breaker.call_async(succeeding)
            assert calls == 1
            assert breaker.state == "CLOSED"

        asyncio.run(_run())

    def test_meta_api_unavailable_error_exists(self):
        from app.services.whatsapp import MetaApiUnavailableError
        assert MetaApiUnavailableError is not None

    def test_send_text_raises_meta_unavailable_on_open_circuit(self):
        """WhatsAppClient lanza MetaApiUnavailableError cuando el circuito está abierto."""
        from app.services.whatsapp import MetaApiUnavailableError
        from app.services.circuit_breaker import CircuitBreakerError
        import app.services.whatsapp as wa_module

        orig_breaker = wa_module._meta_breaker

        class FakeBreaker:
            async def call_async(self, fn, *args, **kwargs):
                raise CircuitBreakerError("test circuit open")

        wa_module._meta_breaker = FakeBreaker()
        try:
            client = WhatsAppClient(token="tok", phone_number_id="pid")

            async def _run():
                with pytest.raises(MetaApiUnavailableError):
                    await client.send_text("59176543210", "hola")
                await client.close()

            asyncio.run(_run())
        finally:
            wa_module._meta_breaker = orig_breaker

    def test_worker_does_not_count_circuit_open_as_attempt(self):
        """MetaApiUnavailableError en el worker usa RETURN_TO_POOL, no apply_backoff_or_fail."""
        import inspect
        from app.jobs.outbox_worker import _send_and_finalize
        src = inspect.getsource(_send_and_finalize)
        assert "MetaApiUnavailableError" in src, \
            "_send_and_finalize debe capturar MetaApiUnavailableError"
        assert "RETURN_TO_POOL" in src, \
            "Circuito abierto debe usar RETURN_TO_POOL sin incrementar intentos"
