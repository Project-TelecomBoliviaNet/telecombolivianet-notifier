...
# US-META: incluye campos para Meta template messages.
# Si MetaTemplateName IS NOT NULL y HsmStatus = 'Aprobada' → enviar como template.
FETCH_PLANTILLA = """
    SELECT
        "Id"               AS id,
        "Texto"            AS texto,
        "HsmStatus"        AS hsm_status,
        "MetaTemplateName" AS meta_template_name,
        "MetaLanguageCode" AS meta_language_code,
        "MetaParamOrder"   AS meta_param_order
...
# FIX-12: INSERT atómico con ON CONFLICT para evitar duplicados entre instancias.
# Requiere índice único parcial (ver migrations/0002_outbox_unique_index.sql):
#   CREATE UNIQUE INDEX uq_outbox_client_tipo_ref
#   ON "NotifOutbox" ("ClienteId", "Tipo", "ReferenciaId") WHERE "EstadoFinal" IS NULL
# RETURNING "Id": si devuelve NULL el recordatorio ya existía (DO NOTHING).
INSERT_OUTBOX_SAFE = """
    INSERT INTO "NotifOutbox"
        ("Id", "Tipo", "ClienteId", "PhoneNumber", "Publicado", "Intentos",
         "EnviarDesde", "EstadoFinal", "CreadoAt", "ContextoJson", "ReferenciaId")
    VALUES
        (gen_random_uuid(), $1, $2, $3, FALSE, 0,
         NOW() + ($4 || ' seconds')::interval, NULL, NOW(), $5::jsonb, $6)
    ON CONFLICT ("ClienteId", "Tipo", "ReferenciaId") WHERE "EstadoFinal" IS NULL
    DO NOTHING
    RETURNING "Id"
"""
...
# FIX-14: libera en bloque todos los registros de un batch que quedaron en limbo
# tras una cancelación (SIGTERM). Solo actúa sobre registros aún no finalizados.
RELEASE_BATCH = """
    UPDATE "NotifOutbox"
    SET "Publicado" = FALSE
    WHERE "Id" = ANY($1)
      AND "EstadoFinal" IS NULL
"""
...
# ── Contexto enriquecido para el worker (cuando ContextoJson = '{}') ──────────
# Permite que mensajes masivos generados sin contexto previo reciban variables reales.
# Se llama solo cuando parse_context() devuelve un dict vacío o con pocas claves.
...
# Facturas pendientes del cliente: para meses_deuda_detalle y variables de deuda
FETCH_INVOICES_FOR_CONTEXT = """
    SELECT
        i."Id"          AS invoice_id,
        i."Amount"      AS amount,
    ...
    WHERE i."ClientId" = $1
      ...
      AND i."Status" IN ('Pendiente', 'Vencida', 'ParcialmentePagada')
    ORDER BY i."DueDate" ASC
"""

# QR activo del cliente (URL de la imagen)
FETCH_CLIENT_QR = """
    SELECT
        q."ImageUrl"    AS image_url,
        q."ExpiresAt"   AS expires_at
    FROM "ClientQrs" q
    WHERE q."ClientId" = $1
      AND q."IsActive"  = TRUE
...
# Configuración del sistema para nombre empresa y portal de pago
FETCH_SYSTEM_CONFIG_BULK = """
    SELECT "Key" AS key, "Value" AS value
    FROM "SystemConfigs"
    WHERE "Key" IN ('ISP:NombreEmpresa', 'ISP:PortalPagoUrl', 'ISP:BackendUrl', 'ISP:FechaCorte')
"""
